...
template<typename K, typename V>
// 二分查找位置
size_t BPlusTree<K, V>::bSearch(std::vector<K> indexs, K key) {
    int l = 0;
    int r = indexs.size() - 1;
    while (l <= r) {
        int mid = (l + r) / 2;
...
template<typename K, typename V>
// 往上更新索引
void BPlusTree<K, V>::up_updata(TreeNode<K, V>* p, std::vector<TreeNode<K, V>*> Stack, int i) {
    for (; i >= 0; i--) {
        // Stack记录p的上层节点
...
template<typename K, typename V>
// 根节点拆分
void BPlusTree<K, V>::rootDivide(TreeNode<K, V>* parent) {
    // 创建新的根节点
...
template<typename K, typename V>
// 构造函数
BPlusTree<K, V>::BPlusTree(int _k): root(nullptr), linkHead(nullptr), k(_k) {}

template<typename K, typename V>
// 查询
bool BPlusTree<K, V>::find(K key, V &data) {
    // 栈记录路径节点
...
template<typename K, typename V>
// 增加节点
bool BPlusTree<K, V>::add(K key, V data) {
    // 记录路径
    std::vector<TreeNode<K, V>*> Stack;
    // 查找叶子节点
    TreeNode<K, V>* leaf = findNode(key, Stack);
...
bool BPlusTree<K, V>::remove(K key) {
    ...
    while (p->keys.size() < ceil(k / 2.0)) {
        ...
        if (lBro && lBro->keys.size() - (ceil(k / 2.0) - p->keys.size()) >= ceil(k / 2.0)) {
            int count = ceil(ceil(k / 2.0) - p->keys.size());
        ...
        if (rBro && rBro->keys.size() - (ceil(k / 2.0) - p->keys.size()) >= ceil(k / 2.0)) {
            size_t Count = ceil(ceil(k / 2.0) - p->keys.size());
...
template<typename K, typename V>
void BPlusTree<K, V>::print(TreeNode<K, V>* tNode) {
    std::cout << "[ ";
